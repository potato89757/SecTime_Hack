import { elizaLogger, getEndpoint, type IAgentRuntime, type IImageDescriptionService, ModelProviderName, models, Service, ServiceType } from "@elizaos/core";
import { AutoProcessor, AutoTokenizer, env, Florence2ForConditionalGeneration, type Florence2Processor, type PreTrainedModel, type PreTrainedTokenizer, RawImage, type Tensor } from "@huggingface/transformers";
...
interface ImageProvider {
  ...
  describeImage(imageData: Buffer, mimeType: string): Promise<{ title: string; description: string }>;
}
...
const handleApiError = async (response: Response, provider: string): Promise<never> => {
  ...
  elizaLogger.error(`${provider} API error:`, response.status, "-", responseText);
  ...
  private async initializeProvider(): Promise<boolean> {
    ...
    if (this.runtime.imageVisionModelProvider) {
      if (this.runtime.imageVisionModelProvider === ModelProviderName.LLAMALOCAL || this.runtime.imageVisionModelProvider === ModelProviderName.OLLAMA) {
        ...
        elizaLogger.warn(`Unsupported image vision model provider: ${this.runtime.imageVisionModelProvider}. ` + `Please use one of the following: ${availableModels}. ` + `Update the 'imageVisionModelProvider' field in the character file.`);
        return false;
      }
    ...
    try {
      ...
      elizaLogger.error(`Failed to initialize the image vision model provider: ${this.runtime.imageVisionModelProvider}`);
      return false;
    }
    return true;
  }
